...
- 内页爬取_完整版.py
  ...
  - 字段与规则：authors 能检测到则写作者姓名；`thinktank_name` 固定“德勤中国（Deloitte）”；附件优先文档（pdf/doc/xls/ppt 等），若无文档而有音/视频则返回其 URL；若文档与音/视频并存仅保留文档 URL。
...
- 内页爬取_完整版.py：
  ...
  - 字段对齐既有格式：title/url/publish_date/authors/thinktank_name/summary/content/attachments。
  - authors：可检测到则写入作者姓名；`thinktank_name` 统一为“安永中国（EY）”。
...
- 新增：麦肯锡中国（McKinsey & Company）（洞察）
  ...
  - 内页爬取_完整版.py：新增 `parse_mck_article` 并在域名分发中接入（mckinsey.com.cn）。解析标题、正文、日期（JSON-LD/Meta/Time 兜底），作者（可检出时使用文章作者姓名），附件遵循“优先文档（pdf/doc/xls/ppt），无文档则音/视频”的规则；`thinktank_name` 统一为“麦肯锡中国（McKinsey & Company）”。
  - 未新增其他文件，沿用原有流程与输出结构。
...
- 新增 JPMorgan Insights（摩根大通研究院）支持：
  ...
  - 详情页：新增 `parse_jpm_article`，实现标题、正文、日期（JSON-LD/Meta/Time）解析；作者优先提取页面作者（JSON-LD 或 meta/DOM），`thinktank_name` 统一为“摩根大通研究院”。
...
- 新增：KPMG 中国（毕马威中国）洞察列表抓取与渲染
  ...
  - 内页爬取_完整版.py：新增 `parse_kpmg_article` 并接入路由（kpmg.com），解析标题、日期（JSON-LD/Meta/Time 兜底）、正文与附件（优先文档，若无文档则音/视频），authors 能检测到则填姓名，`thinktank_name` 统一为“毕马威中国(KPMG)”。
  - 行为保持与既有流程一致，输出结构不变。
...
- 新增：波士顿咨询(BCG)（洞察/Publications）
  ...
  - 内页爬取_完整版.py：新增 `parse_bcg_article` 并接入域名分发（bcg.com），解析标题、正文、日期与作者；
    附件策略与既有一致：优先文档，其次音/视频；并存时仅保留文档；`thinktank_name` 统一为“波士顿咨询（BCG）”。
  - 未新增其他文件，输出结构与现有完全一致。
...
- 内页爬取_完整版.py
  - 新增 `parse_iiss_article`，抽取 `title/url/publish_date/authors/thinktank_name/summary/content/attachments`；`authors` 能检测到则使用文章作者姓名；`thinktank_name` 固定为“国际战略研究所(iiss)”。
